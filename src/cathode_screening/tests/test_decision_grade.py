...
def calibrated_predictions():
    ...
    # In production, load from saved inference results
    np.random.seed(42)
    n = 500
    y = np.clip(np.random.exponential(0.1, n), 0, 1)
    noise = np.random.normal(0, 0.02, n)
    q50 = y + noise
    q10 = q50 - np.abs(np.random.normal(0.03, 0.01, n))
    q90 = q50 + np.abs(np.random.normal(0.03, 0.01, n))
...
def decisions():
    ...
    return ["KEEP"] * 50 + ["MAYBE"] * 300 + ["KILL"] * 150
...
def y_true():
    ...
    np.random.seed(42)
    return np.clip(np.random.exponential(0.1, 500), 0, 1)
